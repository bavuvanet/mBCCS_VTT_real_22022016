...
public class AsynGeneratePPPOEAccount extends AsyncTask<Void, Void, ParseOuput> {
	...
	public AsynGeneratePPPOEAccount(Activity mActivity, View prbIsdn,
	...
	protected void onPostExecute(ParseOuput result) {
		...
		if (result.getErrorCode().equals("0")) {
			...
			currentSub.setAccountPPPOE(result.getAccount());
		} else {
	...
	private ParseOuput generateAccountPPPOE() {
		ParseOuput result;
		String original = "";
		String methodName = "generateAccountPPPOE";
		try {
			...
			rawData.append("<serviceType>" + currentSub.getServiceType()
					+ "</serviceType>");
			rawData.append("<token>" + Session.getToken() + "</token>");
			rawData.append("</input>");
			rawData.append("</ws:" + methodName + ">");
