...
import com.viettel.maps.util.MapConfig.MapType;
...
public class FragmentLoadMap extends GPSTracker implements OnClickListener {

	// init view
    private EditText editBKKS;
	private Button btnkhaosat;
	private MapView mapview;

	private String serviceID = "";
	private String techchologyID = "";
	private String parValueTechology = "";
    ...
    private ArrayList<SurveyOnline> lisSurveyOnlines = new ArrayList<>();
	private SurveyOnline surveyOnlineSelect;

	private SurveyOnline surveyOnlineInit = new SurveyOnline();
	private SysUsersBO sysUsersBOInit = new SysUsersBO();
	...
	private final ArrayList<SurveyOnline> arrSurveyOnlines = new ArrayList<>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Location location = CommonActivity.findMyLocation(FragmentLoadMap.this);
		if (location != null) {
        ...
        Spinner spinner_loaiks = (Spinner) findViewById(R.id.spinner_loaiks);
		Utils.setDataSpinner(FragmentLoadMap.this, lstSpin, spinner_loaiks);
		lndiaban = (LinearLayout) findViewById(R.id.lndiaban);
        EditText edtprovince = (EditText) findViewById(R.id.edtprovince);
		...
		if (!CommonActivity.isNullOrEmpty(province)) {
			...
			try {
				GetAreaDal dal = new GetAreaDal(FragmentLoadMap.this);
				dal.open();
		...
		edtdistrict.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				Intent intent = new Intent(FragmentLoadMap.this, FragmentSearchLocation.class);
				...
				if (item != null) {
					if ("1".equals(item.getId())) {
						...
						if (lat.equals("0") || lon.equals("0")) {
							CommonActivity.createAlertDialog(FragmentLoadMap.this,
		...
		lstMapType.add(new Spin("" + MapType.TRANSPORT, this.getString(R.string.transport)));
		lstMapType.add(new Spin("" + MapType.ADMIN, this.getString(R.string.gsafelite)));
		SharedPreferences preferences = this.getSharedPreferences(
				Define.PRE_NAME, Activity.MODE_PRIVATE);
		String name = preferences.getString(Define.KEY_MENU_NAME, "");
		if(name.contains(";menu_select_googlemap;")){
			lstMapType.add(new Spin(""+MapType.GTRANSPORT, this.getString(R.string.gtransport)));
		}


        Spinner spinner_loaibando = (Spinner) findViewById(R.id.spinner_loaibando);
		Utils.setDataSpinner(FragmentLoadMap.this, lstMapType, spinner_loaibando);
...
//		mapview.setMapTypeControlEnabled(true);
		
		
		CommonActivity.checkConnectionMap(FragmentLoadMap.this);
...
//		MapConfig.changeSRS(SRSType.SRS_4326);
				...
				if (surveyOnlineSelect != null) {
					if (surveyOnlineSelect.getDistance() != null) {
						for (SurveyOnline surveyOnline : lisSurveyOnlines) {
							if (surveyOnline.getDistance().equals(surveyOnlineSelect.getDistance())) {
								arrSurveyOnlines.add(surveyOnline);
							}
						}

					}
					if (!parValueTechology.equals("") && parValueTechology.equals("GPON")) {

						if (arrSurveyOnlines.size() > 1) {
							// show popup
							dialogGPON = new Dialog(FragmentLoadMap.this);
							...
							GetConectorCodeAdapter adapCodeAdapter = new GetConectorCodeAdapter(arrSurveyOnlines,
									FragmentLoadMap.this);
						...
						if (serviceID.equals("F") || serviceID.equals("L") || serviceID.equals("N")) {

							if (arrSurveyOnlines.size() > 1) {

								dialogF = new Dialog(FragmentLoadMap.this);
								dialogF.requestWindowFeature(Window.FEATURE_NO_TITLE);
								dialogF.setContentView(R.layout.connection_layout_lst_conectorcode);
								ListView lstpstn = (ListView) dialogF.findViewById(R.id.lstpstn);
								...
								lstpstn.setOnItemClickListener(new OnItemClickListener() {
									...
									public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
										// dialogF.dismiss();
										showPopupFTTH(arrSurveyOnlines.get(arg2));
									}
								});

								Button btnhuy = (Button) dialogF.findViewById(R.id.btnhuy);
								btnhuy.setOnClickListener(new OnClickListener() {
						...
						if (serviceID.equals("A")) {
							...
							if (arrSurveyOnlines.size() > 1) {
								...
								lstpstn.setOnItemClickListener(new OnItemClickListener() {
									...
									public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
										// dialog.dismiss();
										showPopupADSL(arrSurveyOnlines.get(arg2));
									}
								});

								Button btnhuy = (Button) dialogA.findViewById(R.id.btnhuy);
								btnhuy.setOnClickListener(new OnClickListener() {
						...
						if (serviceID.equals("P")) {
							...
							if (arrSurveyOnlines.size() > 1) {
								...
								lstpstn.setOnItemClickListener(new OnItemClickListener() {
									...
									public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
										// dialog.dismiss();
										showPopupPSTN(arrSurveyOnlines.get(arg2));
									}
								});

								Button btnhuy = (Button) dialogP.findViewById(R.id.btnhuy);
								btnhuy.setOnClickListener(new OnClickListener() {
						...
						if (serviceID.equals("T") || serviceID.equals("I") || serviceID.equals("U")) {

							// show popup
							if (arrSurveyOnlines.size() > 1) {
								dialogT = new Dialog(FragmentLoadMap.this);
								dialogT.requestWindowFeature(Window.FEATURE_NO_TITLE);
								dialogT.setContentView(R.layout.connection_layout_lst_conectorcode);
								ListView lstpstn = (ListView) dialogT.findViewById(R.id.lstpstn);
								...
								Button btnhuy = (Button) dialogT.findViewById(R.id.btnhuy);
								btnhuy.setOnClickListener(new OnClickListener() {
		...
		if (resultCode == Activity.RESULT_OK) {
			...
			case 107:
				if (data != null) {
					...
					// dal.close();
					// } catch (Exception e) {
					// Log.d(FragmentLoadMap.class.getSimpleName(),
					// e.toString());
					// }
					// if (!CommonActivity.isNullOrEmpty(vtmapCode)) {
			...
			case 108:
				if (data != null) {
					...
					try {
						...
						Log.d(FragmentLoadMap.class.getSimpleName(), e.toString());
					}
					if (!CommonActivity.isNullOrEmpty(vtmapCode)) {
						String vtmap = CommonActivity.checkVTmapCode(vtmapCode);
						focusArea(vtmap, type);
	...
	private void showPopupPSTN(SurveyOnline surveyOnline) {
		...
		final Dialog dialog = new Dialog(FragmentLoadMap.this);
		...
		btncall.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				if (txtdtlienhePSTN.getText().toString() != null && !txtdtlienhePSTN.getText().toString().isEmpty()) {
					...
					Toast.makeText(FragmentLoadMap.this, getResources().getString(R.string.checkphone),
							Toast.LENGTH_SHORT).show();
				}
		...
		btnkiemtra.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (txtmahopcap.getText() != null && !txtmahopcap.getText().toString().isEmpty()) {
					if (CommonActivity.isNetworkConnected(FragmentLoadMap.this)) {
				...
				if ((txttnportDLU.getText().toString() != null && !txttnportDLU.getText().toString().isEmpty())
					...
					if (splitntb.length == 2 && (Integer.parseInt(splitntb[0]) < Integer.parseInt(splitntb[1]))) {
						if (splittnhc.length == 2
							...
							txtcheck.setText("NOK");
							Toast.makeText(FragmentLoadMap.this, getResources().getString(R.string.checktnhopcap),
									Toast.LENGTH_SHORT).show();
						}

					} else {
						txtcheck.setText("NOK");
						Toast.makeText(FragmentLoadMap.this, getResources().getString(R.string.checktnportDLU),
								Toast.LENGTH_SHORT).show();
					}
			...
			public void onClick(View v) {
				if (surveyOnlineInit.getConnectorCode() != null && !surveyOnlineInit.getConnectorCode().isEmpty()) {
					if (!txtcheck.getText().toString().isEmpty() && txtcheck.getText().toString().equals("OK")) {
						...
						Bundle mBundle = new Bundle();
						surveyOnlineInit.setSurfaceRadius(editBKKS.getText().toString());
						...
						Toast.makeText(FragmentLoadMap.this, getResources().getString(R.string.checkNotOk),
								Toast.LENGTH_SHORT).show();
					}
			...
			public void onClick(View arg0) {
				...
				if ((txttnport.getText().toString() != null && !txttnport.getText().toString().isEmpty())
					...
					if (splitntb.length == 2 && (Integer.parseInt(splitntb[0]) < Integer.parseInt(splitntb[1]))) {
						...
						txtcheck.setText("NOK");
						Toast.makeText(FragmentLoadMap.this, getResources().getString(R.string.checktnport),
								Toast.LENGTH_SHORT).show();
					}
			...
			public void onClick(View arg0) {
				...
				if (txttntab.getText().toString() != null && !txttntab.getText().toString().isEmpty()) {
					...
					if (splitntb.length == 2 && (Integer.parseInt(splitntb[0]) < Integer.parseInt(splitntb[1]))) {
						...
						txtcheck.setText("NOK");
						Toast.makeText(FragmentLoadMap.this, getResources().getString(R.string.checktntab1),
								Toast.LENGTH_SHORT).show();
					}
				} else {
					txtcheck.setText("NOK");
			...
			public void onClick(View arg0) {
				...
				if (txtsoluongportrong.getText().toString() != null
					...
					if (!txtsoluongportrong.getText().toString().equals("0")) {
						...
						if ("F".equals(serviceID) || "N".equals(serviceID) || "I".equals(serviceID)
								|| "23".equals(groupProductId)) {
							if (txtvendor.getText() != null && !txtvendor.getText().toString().isEmpty()) {
								txtcheck.setText("OK");
							} else {
								txtcheck.setText("NOK");
								Toast.makeText(FragmentLoadMap.this, getResources().getString(R.string.thieuvendor),
										Toast.LENGTH_SHORT).show();
							}
						} else {
							txtcheck.setText("OK");
				...
				if ("STATION".equals(surveyOnline.getConnectorType())) {
					if (txttntbi.getText().toString() != null && !txttntbi.getText().toString().isEmpty()) {
						...
						if (splittntbi.length == 2
							...
							txtcheck.setText("NOK");
							Toast.makeText(FragmentLoadMap.this, getResources().getString(R.string.checktntbi),
									Toast.LENGTH_SHORT).show();
						}
				...
				if ("CONNECTOR".equals(surveyOnline.getConnectorType())) {
					if ((txttnodf.getText().toString() != null && !txttnodf.getText().toString().isEmpty())
						...
						if (splittnOdf.length == 2
							...
							txtcheck.setText("NOK");
							Toast.makeText(FragmentLoadMap.this, getResources().getString(R.string.checktnOdf),
									Toast.LENGTH_SHORT).show();
						}
					} else {
						txtcheck.setText("NOK");
	...
	public void onResume() {
		getSupportActionBar().setTitle(getResources().getString(R.string.manager_customer_connecttion));
		super.onResume();
	}
		...
		case R.id.btn_ks:
			...
			if (serviceID != null || !serviceID.isEmpty()) {
				if (!lat.equals("0.0") && !lon.equals("0.0")) {
					...
					CommonActivity.DoNotLocation(FragmentLoadMap.this);
				}
			} else {
		...
		if (surfaceRadius == null || surfaceRadius.isEmpty()) {
			...
			if (Long.parseLong(surfaceRadius) > 2000) {
				...
				if (CommonActivity.isNetworkConnected(this)) {
					...
					GetListSurfaceOnlineAsyn getListSurfaceOnlineAsyn = new GetListSurfaceOnlineAsyn(this);
	...
	// GPSTracker gps = new GPSTracker(FragmentLoadMap.this);
	//
    ...
    private final OnClickListener moveLogInAct = new OnClickListener() {
		...
		@Override
		public void onClick(View v) {
//			Intent intent = new Intent(FragmentLoadMap.this, LoginActivity.class);
//			startActivity(intent);
//			finish();
			 LoginDialog dialog = new LoginDialog(FragmentLoadMap.this,
                     ";cm.connect_sub_CD;");
	...
	public class GetUserFromCableBoxADSL extends AsyncTask<String, Void, SysUsersBO> {
		...
		protected void onPostExecute(SysUsersBO result) {
			...
			if (errorCode.equals("0")) {
				if (result.getFullname() != null && !result.getFullname().isEmpty()) {
					...
					Dialog dialog = CommonActivity.createAlertDialog(FragmentLoadMap.this,
				...
				if (errorCode.equals(Constant.INVALID_TOKEN2)) {
					...
					Dialog dialog = CommonActivity.createAlertDialog(FragmentLoadMap.this, description,
		...
		private SysUsersBO getUserFromCableBoxADSL(String cableCode) {
			...
			try {
				...
				String response = input.sendRequest(envelope, Constant.BCCS_GW_URL, FragmentLoadMap.this,
		...
		private SysUsersBO getUserFromCableBoxFTTH(String cableCode, String connectorType) {
			...
			try {
				...
				if ("STATION".equals(connectorType)) {
					...
					rawData.append("<infraType>").append(techchologyID);
					rawData.append("</infraType>");
				}
	...
	public class GetListSurfaceOnlineAsyn extends AsyncTask<Void, Void, ArrayList<SurveyOnline>> {
		...
		String errorCode = "";
		String description = "";
		ArrayList<MarkerOptions> arrMarkerOptions = new ArrayList<>();
		...
		protected void onPostExecute(ArrayList<SurveyOnline> result) {
			...
			if (errorCode.equals("0")) {
				...
				if (result.size() > 0) {
					...
					mapview.setCenter(new LatLng(Double.parseDouble(result.get(0).getLat()),
							Double.parseDouble(result.get(0).getLng())));
				} else {
					...
					CommonActivity.createAlertDialog(FragmentLoadMap.this, getResources().getString(R.string.no_data),
							getResources().getString(R.string.app_name)).show();
				}
		...
		private ArrayList<SurveyOnline> getListSurfaceOnline() {
			ArrayList<SurveyOnline> lisSurveyOnlines = new ArrayList<>();
			...
			try {
				...
				if (msType != null && !msType.isEmpty()) {
					if (("23".equals(groupProductId) && "U".equals(serviceID))
							|| ("22".equals(groupProductId) && "U".equals(serviceID))) {
				...
				for (int i = 0; i < nl.getLength(); i++) {
					...
					description = parse.getValue(e2, "description");
					Log.d("description", description);
					nodechild = doc.getElementsByTagName("lstResultSurveyOnlineForm");
					for (int j = 0; j < nodechild.getLength(); j++) {
						Element e1 = (Element) nodechild.item(j);
						...
						surveyOnline.setPolylineData(lat + "_" + lon + ";" + lat1 + "_" + lng1);
						...
						surveyOnline.setSizeKeepDeployAccount(sizeKeepDeployAccount);
						...
						MarkerOptions markerOptions = new MarkerOptions()
								...
								.position(new LatLng(Double.parseDouble(lat1), Double.parseDouble(lng1))).title(address)
			...
			public void onAdminServiceCompleted(AdminServiceResult arg0) {
				if (arg0 != null) {
					if (arg0.getItems() != null && arg0.getItems().size() > 0) {
						...
						CommonActivity
								.createAlertDialog(FragmentLoadMap.this, getResources().getString(R.string.checkareadb),
