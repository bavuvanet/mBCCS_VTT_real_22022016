 ...
 * Author ThinhHQ1
 */
public class FragmentRequestOrderBCCS extends Fragment implements
...
//						+ Long.parseLong(lisresult.get(j).get_sumremane());
//			}
		...
		spindichvu.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,
				...
				viewInFoAdapter = new ViewInFoAdapter(getActivity(),
						lisViewInfoOjectMerges, FragmentRequestOrderBCCS.this,
						FragmentRequestOrderBCCS.this);
	...
	private ArrayList<ViewInfoOjectMerge> getlistDataAsynItem(
		...
		try {
			...
			// if (lisInfoOject != null && lisInfoOject.size() > 0) {
			// if (CacheData.getInstanse().getLisStockOrderDetails() != null
			// && CacheData.getInstanse().getLisStockOrderDetails()
			// .size() > 0) {
			...
			// .getStockModelId());
			// for (int j = 0; j < lisInfoOject.size(); j++) {
			// String stockmodelidStock = String.valueOf(CacheData
			// .getInstanse().getLisStockOrderDetails()
			// .get(i).getStockModelId());
			// String stockmodelidMerge = lisInfoOject.get(j)
			// .get_stock_model_id();
			//
			// if (stockmodelidStock.equals(stockmodelidMerge)) {
			// lisInfoOject.get(j).set_soluong(
			// CacheData.getInstanse()
			// .getLisStockOrderDetails()
			// .get(i).getQuantityOder());
			// }
			// }
			// }
			// }
	...
	private void unit(View view) {
			...
			public void onTextChanged(CharSequence s, int start, int before,
				...
				if (isCheckReview) {
					...
					btnviewreviewoderitem2.setText(getResources().getString(
							R.string.ViewAllItem));
					viewInFoAdapter = new ViewInFoAdapter(getActivity(),
							lisMerges, FragmentRequestOrderBCCS.this,
							FragmentRequestOrderBCCS.this);
					viewInFoAdapter.notifyDataSetChanged();
		...
		if (CacheData.getInstanse().getLisStockOrderDetails() != null) {
			...
			try {
				dal.open();
				// lisInFoStockTH = dal.getListAllStockModelTH();
				// lisInFoStockNV = dal.getListAllStockModelNV();
	...
	public class CancelOrderManager extends AsyncTask<Void, Void, String> {
		...
		public CancelOrderManager(Context context) {
		...
		public String sendRequestCancelOrder() {
			...
			try {
				...
				String response = input.sendRequest(envelope,
						Constant.BCCS_GW_URL,getActivity(),"mbccs_cancelOrderBccs2");
	...
	public class OrderItemManager extends
		...
		public String sendRequestOrderItem(
			...
			try {
				...
				Log.i("LOG", Constant.BCCS_GW_URL);
				String response = input.sendRequest(envelope,
						Constant.BCCS_GW_URL,getActivity(),"mbccs_requestOrderBccs2");
				Log.i("Responseeeeeeeeee", response);
				CommonOutput output = input.parseGWResponse(response);
				// if (!output.getError().equals("0")) {
	...
	public class OrderReItemManager extends
		...
		public String sendRequestOrderItem(
			...
			try {
				...
				Log.i("LOG", Constant.BCCS_GW_URL);
				String response = input.sendRequest(envelope,
						Constant.BCCS_GW_URL,getActivity(),"mbccs_retryRequestOrderBccs2");
				Log.i("Responseeeeeeeeee", response);
				CommonOutput output = input.parseGWResponse(response);
				if (!output.getError().equals("0")) {
					return Constant.ERROR_CODE;
