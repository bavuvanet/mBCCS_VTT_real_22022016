...
public class GetOtpDestroyTransAsyncTask extends
        AsyncTaskCommon<String, String, BankPlusOutput> {

    public GetOtpDestroyTransAsyncTask(Activity context,
    ...
    protected BankPlusOutput doInBackground(String... arg) {
        ...
        return getOtpDestroyTrans(arg[0]);
    }

    private BankPlusOutput getOtpDestroyTrans(String tradingCode) {
        BankPlusOutput result = null;
        try {
            BCCSGateway input = new BCCSGateway();
            input.addValidateGateway("username", Constant.BCCSGW_USER);
            input.addValidateGateway("password", Constant.BCCSGW_PASS);
            input.addValidateGateway("wscode", "mbccs_getOtpDestroyTrans");
            StringBuilder rawData = new StringBuilder();
            rawData.append("<ws:getOtpDestroyTrans>");
            ...
            String response = input.sendRequest(envelope,
                    Constant.BCCS_GW_URL, mActivity, "mbccs_getOtpDestroyTrans");
