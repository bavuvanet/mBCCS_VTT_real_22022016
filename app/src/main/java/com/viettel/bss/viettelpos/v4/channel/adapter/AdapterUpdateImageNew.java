...
public class AdapterUpdateImageNew extends BaseExpandableListAdapter {
	private final LayoutInflater inflater;
	private final Activity activity;
	private final ArrayList<ObjectTypeBO> lstObjectTypeBOs;

	// khai bao 2 interface ----chon thong tin bien hieu
	private final OnChangeInfoSignImage onChangeInfoSignImage;
	private final OnchangeSignImage onchangeSignImage;
	private final OnCancelSignImage onCancelSignImage;

	public interface OnChangeInfoSignImage {
		void onChangeInfoSignImage(ObjectCatBO objectCatBO, int groupPosition, int childPosition);
	}

	// interface change image
	public interface OnchangeSignImage {
		void onChangeSignImage(ObjectCatBO objectCatBO, int groupPosition, int childPosition);
	}

	// interface cancel item update image
	public interface OnCancelSignImage {
		void onCancelSignImage(ObjectCatBO objectCatBO, int groupPosition, int childPosition);
	}

	public AdapterUpdateImageNew(Activity activity,
			ArrayList<ObjectTypeBO> arrObjectTypeBOs,
			OnChangeInfoSignImage onChangeInfoSignImage,
			OnchangeSignImage onchangeSignImage,
			OnCancelSignImage onCancelSignImage) {
	...
	public View getChildView(final 
		...
		if (objectCatBO.getBmpImage() != null) {
			...
			if (objectCatBO.getHaveImage().equals("false")) {
				...
				imgDB.setBackgroundResource(R.drawable.logo_vt);
			} else {
		...
		txtnameDB.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				onChangeInfoSignImage.onChangeInfoSignImage(objectCatBO,groupPosition,childPosition);
			}
		});
		...
		imgDB.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (!CommonActivity.isNullOrEmpty(objectCatBO.getRequestType())) {
					onchangeSignImage.onChangeSignImage(objectCatBO,groupPosition,childPosition);
				} else {
		...
		btnDelete.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View arg0) {
				onCancelSignImage.onCancelSignImage(objectCatBO,groupPosition,childPosition);
			}
		});
