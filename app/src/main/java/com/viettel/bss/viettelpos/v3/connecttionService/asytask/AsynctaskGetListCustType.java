...
public class AsynctaskGetListCustType extends AsyncTask<Void, Void, ParseOuput> {
	private Activity mActivity = null;
	private View prbCustType;
	private View btnRefreshCustType;
	private Spinner spnCustType;

	public AsynctaskGetListCustType(Activity mActivity, View prbCustType,
...
//			lst.add(0, reason);
			...
			if(!CommonActivity.isNullOrEmptyArray(lst)){
				for (OptionSetValueDTO item: lst) {
					if("1".equals(item.getValue())){
						spnCustType.setSelection(lst.indexOf(item));
	...
	private ParseOuput getListCustType() {
		...
		try {
			...
			String response = input.sendRequest(envelope, Constant.BCCS_GW_URL,
					mActivity, "mbccs_getLsOptionSetValueByCode");
